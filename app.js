...
function selectedPriceRange(){
  ...
  if(min>max-50000){
    if(document.activeElement===minEl) min=max-50000;
    else max=min+50000;
  }
...
function updatePriceRange(){
  ...
  $("priceRangeLabel").textContent=`S$${(min/1000000).toFixed(2)}M – S$${(max/1000000).toFixed(2)}M`;
  $("rangeFill").style.left=`${((min-1000000)/span)*100}%`;
  $("rangeFill").style.right=`${100-((max-1000000)/span)*100}%`;
}
function matchesPriceRange(d,min,max){
...
function applyFilters(){
  ...
  const q=$("searchInput").value.trim().toLowerCase(),region=$("regionFilter").value;
...
function applyPreset(name){
  ...
  ["tenureFilter","topFilter"].forEach(id=>$(id).value="");
...
async function init(){
  ...
  if($("resetBtn")) $("resetBtn").onclick=()=>{["searchInput","regionFilter","categoryFilter","riskFilter","tenureFilter","topFilter"].forEach(id=>$(id).value="");$("priceMin").value=1000000;$("priceMax").value=3000000;$("scoreFilter").value=0;$("scoreValue").textContent="0";applyPreset("overall")};
